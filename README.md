...
메인 페이지는 브랜드 소개 → 데이터랩 4종 → 추천 자료와 분석 목록 → 열심남 소개 → 채널 링크 순서입니다.
PC 데이터랩은 2×2 배치이며 모바일에서는 1열로 표시됩니다.
분석자료의 `analysis-featured` 클래스는 입문용 추천 자료를 강조하며, 다른 자료와 동일하게 주제 필터가 적용됩니다.
...
추천 자료를 바꾸려면 기존 카드의 `analysis-featured` 클래스와 `featured-label`, `featured-arrow` 요소를 새 추천 카드로 옮기고 목록 맨 앞에 배치하세요. 날짜는 실제 자료 기준 시점만 기재합니다.
